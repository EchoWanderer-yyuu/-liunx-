...
void *read_from_client_then_write(void *arg)
{
    ...
    //当客户端输入ctrl+d时会退出循环
    shutdown(client_fd,SHUT_RD);  //其实不用，如果有服务器发消息的功能要加，关闭写功能
    close(client_fd);
    free(read_buf);
    free(write_buf);
}
...
int main(int argc, char const *argv[])
{
    ...
    while(1)
    {
        ...
        printf("与客户端%s  %d 建立链接%d\n",inet_ntoa(client_addr.sin_addr),ntohs(client_addr.sin_port),clientfd);
        //和每一个客户端使用一个线程交互，把客户端发送的信息打印到控制台 回复收到
        if(pthread_createw(&pid_read_write,NULL,read_from_client_then_write,(void *)&clientfd))
        {
